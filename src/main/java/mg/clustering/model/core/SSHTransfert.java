...
public class SSHTransfert extends Transfert {
  public SSHTransfert(String ipv4, String username, String password,
                      String port) {
  ...
  Session getSession() {
    ...
    try {
      ...
      throw new RuntimeException("Error while connecting to SSH server " +
                                 getUsername() + "@" + getHost() + ":" +
                                 getPort());
    }
    return session;
  ...
  public void upload(String sourcePath, String destinationPath) {
    try {
      ...
      throw new RuntimeException("Error while transferring file via SSH to " +
  ...
  public void execute(String command) {
    try {
      ...
      throw new RuntimeException("Error while executing command via SSH to " +
                                 getHost() + " : " + e.getMessage());
    }
  }
}
