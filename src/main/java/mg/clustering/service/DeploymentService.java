...
public class DeploymentService {
  ...
  public DeploymentService(
      ...
      BuildRepository buildRepository, ServerRepository serverRepository,
  ...
  public void makeBuild(Build build) {
    ...
    if (!build.cloneGit())
      throw new RuntimeException("Error while cloning git repository " +
                                 build.getGitUrl());
  }

  @Transactional
  public void deploy(long buildId, String configFile, String content,
                     Long[] serverList, Long[] serverApplicationList,
                     Long[] transfertMethodList) {
    Build build = buildRepository.findById(buildId).orElseThrow(
        () -> new RuntimeException("Build to deploy not found"));

    if (serverList == null || serverList.length == 0 ||
        serverApplicationList == null || serverApplicationList.length == 0 ||
        transfertMethodList == null || transfertMethodList.length == 0)
    ...
    for (int i = 0; i < serverList.length; i++) {
      ...
      deployment.setServer(
          serverRepository.findById(serverList[i]).orElseThrow());
      deployment.setServerApplication(
          serverApplicationRepository.findById(serverApplicationList[i])
              .orElseThrow());
      deployment.setTransfertMethod(
          transfertMethodRepository.findById(transfertMethodList[i])
              .orElseThrow());
