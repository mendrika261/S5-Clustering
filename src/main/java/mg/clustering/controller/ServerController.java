...
public class ServerController {
  ...
  public String addServer(@ModelAttribute("server") Server server,
                          Model model) {
    List<OperatingSystem> operatingSystemList =
        operatingSystemRepository.findAll(
            Sort.by("operatingSystemType", "name"));
  ...
  public String addServer(@ModelAttribute Server server,
    ...
    try {
      ...
      redirectAttributes.addFlashAttribute(
          "errorMessage",
          "Server with IPv4: " + server.getIpv4() + " already exists");
    } catch (Exception e) {
  ...
  public String addServerApp(@ModelAttribute("serverApplication")
                             ServerApplication serverApplication,
                             @PathVariable long serverId, Model model) {
    Optional<Server> serverOptional = serverRepository.findById(serverId);
    if (serverOptional.isEmpty())
      return "redirect:/servers/add";

    List<ServerApplicationType> serverApplicationTypeList =
        serverApplicationTypeRepository.findAll(Sort.by("name", "version"));
  ...
  public String
  addServerApp(@PathVariable long serverId,
               @ModelAttribute ServerApplication serverApplication,
               BindingResult bindingResult,
               RedirectAttributes redirectAttributes) {
    redirectAttributes.addFlashAttribute("serverApplication",
                                         serverApplication);
    if (bindingResult.hasErrors()) {
      redirectAttributes.addFlashAttribute(
          "errorMessage",
    ...
    try {
      ...
      redirectAttributes.addFlashAttribute(
          "errorMessage", "Server application with port: " +
                              serverApplication.getPort() + " already exists");
    } catch (Exception e) {
  ...
  public String addTransfertMethod(@PathVariable long serverId, Model model,
                                   @ModelAttribute("transfertMethod")
  ...
  public String
  addTransfertMethod(@PathVariable long serverId,
    ...
    try {
      ...
      redirectAttributes.addFlashAttribute(
          "errorMessage",
          "Transfert method for " +
              transfertMethod.getTransfertMethodType().getName() +
              " and credential " + transfertMethod.getUsername() + "@" +
              transfertMethod.getServer().getIpv4() + ":" +
              transfertMethod.getPort() + " already exists");
    } catch (Exception e) {
      redirectAttributes.addFlashAttribute("errorMessage", e.getMessage());
    }
    return "redirect:/servers/{serverId}/transfert-methods/add";
